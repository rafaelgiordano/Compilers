...
public class Compiler {
    ...
    private boolean weAreInMain = false;
    ...
    */
    ...
    private SubDclList subdcls() {
		ArrayList<SubDcl> subdclList = null;
		subdclList = new ArrayList<SubDcl>();

		subdclList.add(subdcl());
		...
		return new SubDclList(subdclList);
	}
	...
	private Subhead subhead() {
        ...
        if (lexer.token == Symbol.FUNCTION) {
                ...
                // Análise Semântica
                // verifica se a função de nome 'name' já foi declarada
                if (symbolTable.putInGlobal(name, new Function(name, args, type)) != null) {
                        lexer.error("Função " + name + " já foi declarada.\n");
                }

                return new Function(name, args, type);
        }
                ...
                // Análise Semântica
                // verifica se o procedimento de nome 'name' já foi declarado
                if (symbolTable.putInGlobal(name, new Procedure(name, args)) != null) {
                        lexer.error("Procedimento " + name + " já foi declarado.\n");
                }

                return new Procedure(name, args);
        }
        else {
                lexer.error("'function' ou 'procedure' esperado.\n");
        }
    ...
    */
    ...
    private Dcl dcl(){
        ...
        // Insere as variáveis nas tabelas hash
        for (String v : idlist) {
                if (weAreInMain) {
                    if (symbolTable.putInGlobal(v, type) != null) {
                            lexer.error("Variável " + v + " já foi declarada.\n");
                    }
                }
                else {
        ...
        switch(lexer.token){
            case IDENT:
                if (symbolTable.get(lexer.getStringValue()) instanceof Procedure) {
                    // Temos uma chamada para um procedimento
    ...
    */
    private Variable vb(){
        ...
        if((my_type = (Type) symbolTable.get(id)) == null)
            lexer.error("Variable["+id+"]not declared");
        //if((my_type = (Type) symbolTable.getInGlobal(id)) == null)
          //  lexer.error("Variable["+id+"]not declared");
        return new Variable (id, expr, my_type);
    }
        ...
        switch (lexer.token){
            case IDENT:
                if (symbolTable.get(lexer.getStringValue()) instanceof Function) {
                    // Temos uma chamada para uma função
                    stmt = functioncall();
                }else{
                vb = vb(); 
                }
    ...
    */
    ...
    private FunctionCall functioncall() {
        ...
        name = id();
        // Procura na tabela global o procedimento de nome 'name'
        // Se não achar, significa que não foi declarado. Um erro deve ser emitido.
        if ((f = (Function) symbolTable.get(name)) == null) {
                lexer.error("Função " + name + " não foi declarada.\n");
        }
        ...
        return new FunctionCall(f, exprList);
    }
    ...
    private ProcedureCall procedurecall() {
        ...
        // Procura na tabela global o procedimento de nome 'name'
        // Se não achar, significa que não foi declarado. Um erro deve ser emitido.
        if ((p = (Procedure) symbolTable.get(name)) == null) {
                lexer.error("Procedimento " + name + " não foi declarado.\n");
        }
        ...
        return new ProcedureCall(p, exprList);
    }
